...
type geometry_xml
   ...
   integer, dimension(:), pointer                  :: xgrid => null()
   integer, dimension(:), pointer                  :: ygrid => null()
   integer, dimension(:), pointer                  :: zgrid => null()
...
type mat_xml
   integer                                         :: uid
   real(kind=kind(1.0d0)), dimension(:), pointer   :: totalxs => null()
...
subroutine read_xml_type_geometry_xml( info, starttag, endtag, attribs, noattribs, data, nodata, &
   ...
   do
      ...
      case('xgrid')
         call read_xml_integer_array( &
            info, tag, endtag, attribs, noattribs, data, nodata, &
            dvar%xgrid, has_xgrid )
      case('ygrid')
         call read_xml_integer_array( &
            info, tag, endtag, attribs, noattribs, data, nodata, &
            dvar%ygrid, has_ygrid )
      case('zgrid')
         call read_xml_integer_array( &
            info, tag, endtag, attribs, noattribs, data, nodata, &
            dvar%zgrid, has_zgrid )
      case('uniform')
   ...
   if ( .not. has_xgrid ) then
      has_dvar = .false.
      call xml_report_errors(info, 'Missing data on xgrid')
   endif
   if ( .not. has_ygrid ) then
      has_dvar = .false.
      call xml_report_errors(info, 'Missing data on ygrid')
   endif
   if ( .not. has_zgrid ) then
      has_dvar = .false.
      call xml_report_errors(info, 'Missing data on zgrid')
   endif
...
subroutine write_xml_type_geometry_xml( &
   ...
   call write_to_xml_integer_array( info, 'xgrid', indent+3, dvar%xgrid)
   call write_to_xml_integer_array( info, 'ygrid', indent+3, dvar%ygrid)
   call write_to_xml_integer_array( info, 'zgrid', indent+3, dvar%zgrid)
...
subroutine read_xml_type_mat_xml( info, starttag, endtag, attribs, noattribs, data, nodata, &
   ...
   if ( .not. has_totalxs ) then
      has_dvar = .false.
      call xml_report_errors(info, 'Missing data on totalxs')
   endif
...
subroutine write_xml_type_mat_xml( &
   ...
   call write_to_xml_double_array( info, 'totalxs', indent+3, dvar%totalxs)
